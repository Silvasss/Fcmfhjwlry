...
const AddSchema = Yup.object().shape({
    ...
    responsabilidade: Yup.string().max(150).required('Responsabilidade é obrigatório')
});
...
export default function Edit({ copia, handleClose, open, updateDados }) {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            ...
            await endpoints.customFetch.put('/usuario/experiencia', event, { headers: { Authorization: `Bearer ${user.token}` } })
    ...
    return (
        ...
        >
            ...
            <Formik
                initialValues={{
                    experiencia_Id: copia.experiencia_Id,
                    setor: copia.setor,
                    empresa: copia.empresa,
                    plusCode: copia.plusCode,
                    vinculo: copia.vinculo,
                    funcao: copia.funcao,
                    responsabilidade: copia.responsabilidade,
                    status: copia.ativo,
                    fim: copia.fim,
                    inicio: copia.inicio
                }}
                                ...
                                <Grid container spacing={3}>
                                    ...
                                    <Grid item sm={6} xs={12}>
                                        <Stack spacing={1}>
                                            ...
                                            <LocalizationProvider dateAdapter={AdapterDayjs}>
                                                <DatePicker id="fim" disabled={values.status} views={['day', 'month', 'year']} defaultValue={dayjs(values.fim)} onChange={(e) => handleChange({ target: { name: "fim", value: e } })} slotProps={{ field: { clearable: true } }} />
                                            </LocalizationProvider>
                                        </Stack>
