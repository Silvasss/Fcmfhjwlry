...
export default function Edit({ copia, handleClose, open, updateDados, user, endpoints  }) {
    ...
    const handleSubmit = async (event) => {
        try {
            ...
            await endpoints.customFetch.put('/instituicao/curso', event, { headers: { Authorization: `Bearer ${user.token}` } })
                    ...
                    <Form noValidate className='form' method='POST' onSubmit={handleSubmit}>
                        ...
                        <DialogActions disableSpacing sx={{ m: 1 }}>
                            <Stack display={"flex"} flexDirection={"row"} justifyContent={"flex-end"} alignItems={"center"}>
                                ...
                                <Button variant="contained" type="submit" disabled={!isValid || !dirty || isSubmitting} sx={{ ml: '16px' }}>Salvar</Button>
                            </Stack>
                        </DialogActions>
                    </Form>
                )}
